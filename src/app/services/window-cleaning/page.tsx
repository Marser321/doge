...
import { ArrowLeft, Droplets, Camera, FileText, Upload, X, ImageIcon, Film, Send, CheckCircle, ShieldCheck } from 'lucide-react'
...
export default function WindowCleaningPage() {
  ...
  const handleSubmit = () => {
    ...
    const message = encodeURIComponent(
      `Hola DOGE.S.M LLC, solicito un estimado para Limpieza de Cristales.\n\n` +
      `📸 Descripción: ${desc}\n` +
      `👤 Nombre: ${name}\n` +
      `📞 Contacto: ${contact}\n` +
      `📍 Dirección: ${address}\n` +
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 md:py-16 relative z-10">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-accent/20 bg-accent/5 text-accent text-[10px] font-black uppercase tracking-[0.3em] mb-8">
            <ShieldCheck className="w-4 h-4" /> {lang === 'es' ? 'Sin Suscripción Necesaria' : 'No Subscription Needed'}
          </span>
          <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tighter uppercase leading-[1.1] text-foreground font-michroma">
            {t('wc.title')} <br className="hidden md:block" /> <span className="silver-text">{t('wc.title2')}</span>
          </h1>
        ...
        <ServiceVisualBanner visualId="windowCleaning" lang={lang} />
        ...
        >
          <div className="inline-flex rounded-2xl border border-accent/10 bg-foreground/5 p-1.5">
            ...
            >
              <Camera className="w-4 h-4" /> {t('wc.methodPhotos')}
            </button>
            <button
            ...
            >
              <FileText className="w-4 h-4" /> {t('wc.methodText')}
            </button>
          </div>
        </motion.div>
              ...
              {method === 'photos' ? (
                ...
                >
                  ...
                  <textarea
                    ...
                    placeholder={t('wc.textPlaceholder')}
                    rows={8}
